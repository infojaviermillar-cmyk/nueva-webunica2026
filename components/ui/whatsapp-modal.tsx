    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6 bg-zinc-950/85 backdrop-blur-md animate-in fade-in duration-300">
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto custom-scrollbar">
          {/* Header Visual */}
          <div className="bg-zinc-950 p-6 sm:p-8 text-center relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#25d366]/20 blur-[60px] rounded-full" />
            <div className="w-16 h-16 mx-auto mb-4 relative">
              <div className="relative w-16 h-16 rounded-full overflow-hidden shadow-2xl border-2 border-white/10">
                 <img 
              ...
              <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-[#25d366] rounded-full border-2 border-white flex items-center justify-center shadow-lg">
            ...
            <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tighter">Continuar por WhatsApp</h2>
            <p className="text-zinc-500 text-xs sm:text-sm mt-2 font-light">Completa tus datos para iniciar la conversación con un asesor.</p>
          </div>

          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-4">
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1 ml-3">Nombre Completo</label>
              <input 
                required
                type="text" 
                value={formData.name}
                onChange={(e) => setFormData({...formData, name: e.target.value})}
                className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-5 py-3 sm:py-3.5 focus:outline-none focus:border-[#25d366] transition-all placeholder:text-zinc-300 text-zinc-950 font-bold"
                placeholder="Ej: Javier Millar"
              />
            </div>

            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1 ml-3">Tu Email de contacto</label>
              <input 
                required
                type="email" 
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1 ml-3">WhatsApp / Teléfono</label>
              <input 
                required
                type="tel" 
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1 ml-3">Servicio de Interés</label>
              <div className="relative">
                <select 
                  className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-5 py-3 sm:py-3.5 focus:outline-none focus:border-[#25d366] transition-all text-zinc-950 font-bold appearance-none cursor-pointer"
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full py-4 sm:py-4.5 bg-[#25d366] text-white rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl shadow-[#25d366]/30 hover:bg-[#20bd5a] transition-all scale-100 hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 mt-3"
            >
              {loading ? (
        ...
        {/* Footer info */}
        <div className="bg-zinc-50 p-4 sm:p-5 text-center border-t border-zinc-100 flex items-center justify-center gap-2">
