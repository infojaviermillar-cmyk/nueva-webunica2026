...
export default function WhatsAppModal({ isOpen, onClose }: WhatsAppModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) {
      setFormData({ interest: ALL_SERVICES[0], name: "", email: "", phone: "" });
    }
  }, [isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Intentamos abrir en una nueva pestaña, si falla usamos la misma (evita bloqueos de popups)
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6 bg-zinc-950/85 backdrop-blur-md animate-in fade-in duration-300">
      <div 
        className="bg-white w-full max-w-lg rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col max-h-[92vh] animate-in zoom-in-95 duration-300"
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto custom-scrollbar">
          {/* Header Visual */}
          <div className="bg-zinc-950 p-4 sm:py-5 sm:px-8 text-center relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#25d366]/20 blur-[60px] rounded-full" />
            <div className="w-12 h-12 mx-auto mb-2 relative">
              <div className="relative w-12 h-12 rounded-full overflow-hidden shadow-2xl border-2 border-white/10">
                 <img 
              ...
              <div className="absolute -bottom-0.5 -right-0.5 w-4.5 h-4.5 bg-[#25d366] rounded-full border border-white flex items-center justify-center shadow-lg">
            ...
            <h2 className="text-lg sm:text-xl font-black text-white uppercase tracking-tighter">Continuar por WhatsApp</h2>
            <p className="text-zinc-500 text-[11px] sm:text-xs mt-1 font-light">Completa tus datos para iniciar la conversación con un asesor.</p>
          </div>

          <form onSubmit={handleSubmit} className="p-5 sm:px-8 sm:py-6 space-y-3">
            <div>
              <label className="block text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-0.5 ml-3">Nombre Completo</label>
              <input 
                required
                type="text" 
                value={formData.name}
                onChange={(e) => setFormData({...formData, name: e.target.value})}
                className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-5 py-2.5 sm:py-3 focus:outline-none focus:border-[#25d366] transition-all placeholder:text-zinc-300 text-zinc-950 font-bold text-sm"
                placeholder="Ej: Javier Millar"
              />
            </div>

            <div>
              <label className="block text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-0.5 ml-3">Tu Email de contacto</label>
              <input 
                required
                type="email" 
            ...
            <div>
              <label className="block text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-0.5 ml-3">WhatsApp / Teléfono</label>
              <input 
                required
                type="tel" 
            ...
            <div>
              <label className="block text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-0.5 ml-3">Servicio de Interés</label>
              <div className="relative">
                <select 
                  className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-5 py-2.5 sm:py-3 focus:outline-none focus:border-[#25d366] transition-all text-zinc-950 font-bold appearance-none cursor-pointer text-sm"
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full py-3.5 sm:py-4 bg-[#25d366] text-white rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl shadow-[#25d366]/30 hover:bg-[#20bd5a] transition-all scale-100 hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 mt-2"
            >
              {loading ? (
        ...
        {/* Footer info */}
        <div className="bg-zinc-50 p-3 sm:p-3.5 text-center border-t border-zinc-100 flex items-center justify-center gap-2">
