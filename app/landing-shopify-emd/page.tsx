...
export default function ShopifyEmdLandingPage() {
  ...
  const plans = [
    {
      name: "Plan Inicial",
      desc: "Ideal para marcas que están comenzando.",
      features: [
        "Configuración de theme gratuito",
        "Hasta X colecciones principales",
        "Pasarelas de pago y envíos base",
        "Configuración de páginas legales"
      ]
    },
    {
      name: "Plan Profesional",
      desc: "Para negocios que buscan destacar y escalar.",
      recommended: true,
      features: [
        "Instalación y personalización de Theme Premium",
        "Diseño de banners y gráficas principales",
        "Optimización de fichas de producto para conversión",
        "Instalación de apps de marketing (Reseñas, WhatsApp)"
      ]
    },
    {
      name: "Plan Avanzado",
      desc: "Soluciones a medida con funciones complejas.",
      features: [
        "Diseño UX/UI personalizado",
        "Automatizaciones de correos y carritos abandonados",
        "Integraciones con sistemas externos (Inventarios/ERP)",
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               {plans.map((p, i) => (
                 <div key={i} className={`relative bg-zinc-50 rounded-[3rem] p-10 flex flex-col transition-all duration-300 ${p.recommended ? 'border-2 border-pink-600 shadow-2xl shadow-pink-600/10 scale-100 lg:scale-105 z-10' : 'border border-zinc-200'}`}>
                    ...
                    <h3 className="text-2xl font-black mb-3 uppercase text-zinc-900">{p.name}</h3>
                    <p className="text-sm text-zinc-500 font-light mb-8 h-10">{p.desc}</p>
