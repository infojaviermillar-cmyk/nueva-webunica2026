  ...
  return (
    <div className="min-h-screen bg-white font-sans antialiased text-zinc-900 overflow-x-hidden">
      <main className="pt-32">
        {/* Hero */}
        <section id="inicio" className="relative max-w-7xl mx-auto px-6 py-20 lg:py-40 text-center">
          <div className="absolute top-0 right-1/2 translate-x-1/2 w-[600px] h-[600px] bg-pink-600/10 blur-[120px] rounded-full -z-10" />
          <div className="max-w-4xl mx-auto">
            <div>
              <div className="inline-block px-4 py-1.5 mb-8 text-[10px] font-black tracking-[0.2em] text-pink-600 uppercase bg-pink-600/10 rounded-full border border-pink-600/20">
              ...
              <h1 className="text-5xl lg:text-[90px] font-black tracking-tighter leading-[0.8] mb-10 uppercase text-zinc-950">
                Desarrollo <br/><span className="text-pink-600 italic font-serif lowercase font-light">Shopify</span> en Chile
              </h1>
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center lg:justify-start">
                <a 
                  href="#planes"
                  className="px-10 py-5 bg-pink-600 text-white rounded-[2rem] font-black uppercase tracking-[0.15em] text-[10px] hover:bg-pink-700 transition-all shadow-2xl shadow-pink-600/30 scale-100 hover:scale-105 active:scale-95 text-center flex items-center justify-center gap-2"
                >
        ...
        <section id="ventajas" className="bg-zinc-950 py-32 rounded-[4rem] mx-4 text-white overflow-hidden relative">
          <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_center,rgba(219,39,119,0.1),transparent)] pointer-events-none" />
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
              <h2 className="text-4xl lg:text-6xl font-black mb-8 tracking-tighter uppercase leading-tight">
                Integración a nivel <br/><span className="text-pink-600">Profesional</span>
              </h2>
              ...
              <ul className="space-y-6">
                {[
                  ...
                  <li key={i} className="flex gap-4">
                     <div className="w-12 h-12 rounded-2xl bg-pink-600/10 border border-pink-600/20 flex items-center justify-center flex-shrink-0 text-pink-600 font-bold">0{i+1}</div>
                     <div>
              ...
              <a href="#planes" className="inline-block mt-12 text-pink-600 font-bold uppercase tracking-widest text-sm hover:text-white transition-colors">
        ...
        <section id="planes" className="py-32 bg-white">
           <div className="max-w-7xl mx-auto px-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                 {plans.map((p, i) => (
                   <div key={i} className={`relative bg-zinc-50 rounded-[3rem] p-10 border transition-all duration-300 ${p.recommended ? 'border-pink-600 ring-4 ring-pink-600/10' : 'border-zinc-200'}`}>
                      <h3 className="text-2xl font-black mb-2 uppercase">{p.name}</h3>
                      <div className="text-4xl font-black mb-8">{p.price}</div>
                      <ul className="space-y-4 mb-10">
                         {p.features.map((f, idx) => (
                           <li key={idx} className="text-sm text-zinc-600 flex gap-3"><span className="text-pink-600 font-black">✓</span>{f}</li>
                         ))}
                      </ul>
                      <a 
                        href="#contacto" 
                        className={`w-full block text-center py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition-colors ${p.recommended ? 'bg-pink-600 text-white hover:bg-pink-700' : 'bg-zinc-200 text-zinc-600 hover:bg-zinc-300'}`}
                      >
                        {p.cta}
                      </a>
                   </div>
