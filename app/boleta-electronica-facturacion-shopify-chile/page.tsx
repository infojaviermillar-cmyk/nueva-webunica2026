    ...
    <main className="min-h-screen bg-zinc-950 pt-32 pb-24 overflow-hidden font-sans">
      ...
      <section className="max-w-7xl mx-auto px-6 mb-32">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-black text-white uppercase tracking-tight">Sistemas Ligeros de Facturación</h2>
          <p className="text-zinc-400 mt-4 max-w-2xl mx-auto">Alternativas más económicas y directas que un ERP completo. Diseñadas específicamente para cumplir con el SII de forma rápida.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {billingSystems.map((sys, idx) => (
            <div key={idx} className={`p-8 rounded-[2rem] border relative transition-all duration-300 flex flex-col ${sys.recommended ? 'bg-zinc-900/80 border-violet-500/30 shadow-[0_0_30px_rgba(139,92,246,0.05)]' : 'bg-white/5 border-white/10'}`}>
              
              {sys.recommended && (
                <div className="absolute top-0 right-0 px-3 py-1.5 bg-violet-600 text-white text-[9px] font-black uppercase tracking-widest rounded-bl-2xl rounded-tr-[2rem]">
                  Top Pick
                </div>
              )}

              <div className="text-4xl mb-4">{sys.logo}</div>
              <h3 className="text-2xl font-black text-white mb-1 tracking-tight">{sys.name}</h3>
              <div className="text-violet-400 text-[10px] font-bold uppercase tracking-widest mb-4">{sys.type}</div>
              
              <div className="inline-block bg-zinc-950 px-3 py-1.5 rounded-lg border border-zinc-800 text-zinc-300 text-xs font-medium mb-6 w-fit">
                {sys.pricing}
              </div>

              <div className="mb-6 flex-grow">
                <div className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-3">Beneficios</div>
                <ul className="space-y-3">
                  {sys.pros.map((p, i) => (
                    <li key={i} className="flex gap-2 text-sm text-zinc-300 font-light leading-snug">
              ...
              <div>
                <div className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-3">Limitaciones</div>
                <ul className="space-y-3">
                  {sys.cons.map((c, i) => (
                    <li key={i} className="flex gap-2 text-sm text-zinc-400 font-light leading-snug">
