...
export default function CoursesPage() {
  ...
  return (
    <main className="min-h-screen bg-zinc-950 pt-32 pb-24 font-sans">
      ...
      <section className="max-w-7xl mx-auto px-6 mb-32">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {courses.map((course, idx) => (
            <div key={idx} className={`p-8 md:p-10 bg-gradient-to-br ${course.color} bg-zinc-900/40 border ${course.borderColor} rounded-[2.5rem] relative flex flex-col`}>
              
              <div className={`absolute top-0 right-0 px-4 py-2 bg-zinc-950/50 backdrop-blur-md border-b border-l ${course.borderColor} ${course.textColor} text-[9px] font-black uppercase tracking-widest rounded-bl-3xl rounded-tr-[2.5rem]`}>
                {course.badge}
              </div>

              <div className="text-5xl mb-6">{course.icon}</div>
              <h3 className="text-2xl font-black text-white tracking-tight leading-none mb-3">{course.title}</h3>
              
              <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-zinc-950 rounded-lg border border-zinc-800 text-zinc-300 text-[11px] font-bold uppercase tracking-wider mb-6 w-fit">
                ⏱️ {course.duration}
              </div>

              <p className="text-zinc-400 leading-relaxed font-light text-sm mb-8 flex-grow">
                {course.description}
              </p>

              <div className="mt-auto">
                <div className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-4">Temario Principal</div>
              ...
              <LeadButton className={`w-full py-4 text-center border ${course.borderColor} ${course.textColor} hover:bg-white/5 rounded-2xl font-bold text-xs uppercase tracking-wider transition-colors`}>
      ...
      <section className="max-w-5xl mx-auto px-6 mb-32">
        <div className="bg-zinc-900 border border-zinc-800 p-10 md:p-16 rounded-[3rem] text-center">
          <h2 className="text-3xl font-black text-white mb-6 uppercase tracking-tight">Metodología de Enseñanza</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
            <div>
              <div className="w-12 h-12 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-xl mb-4">🎥</div>
              <h3 className="text-lg font-bold text-white mb-2">Sesiones Grabadas</h3>
              <p className="text-zinc-500 text-sm font-light leading-relaxed">Cada clase queda grabada en la nube y te la entregamos para que quede como material de inducción para futuros empleados.</p>
            </div>
            <div>
              <div className="w-12 h-12 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-xl mb-4">🛠️</div>
              <h3 className="text-lg font-bold text-white mb-2">Hands-on (Práctico)</h3>
              <p className="text-zinc-500 text-sm font-light leading-relaxed">Compartimos pantalla y te hacemos hacer clic. Aprendes haciendo sobre tu propio catálogo y tus propios clientes reales.</p>
            </div>
            <div>
              <div className="w-12 h-12 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-xl mb-4">🙋‍♂️</div>
              <h3 className="text-lg font-bold text-white mb-2">Preguntas y Respuestas</h3>
              <p className="text-zinc-500 text-sm font-light leading-relaxed">A diferencia de un curso en Udemy, aquí puedes interrumpir al instructor para resolver los problemas específicos de tu rubro.</p>
            </div>
          </div>
        </div>
      </section>
